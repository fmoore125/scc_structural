...
is.skewnormal <- function(mu, qs, as) {
    return((!is.na(mu) && length(qs) == 2 && all(qs != .5) & !is.bounded(qs)) ||
...
generate.pdf <- function(mu, qs, as, N) {
    ...
    if (is.skewnormal(mu, qs, as)) {
        ...
        result1 <- optim(c(xi0, omega0, alpha0), function(params) {
            ...
            as.pred <- qsn(qs, params[1], abs(params[2]), params[3])
    ...
    ## Take an average sd
    mus <- ifelse(!is.na(mu), mu,
           ifelse(any(qs == .5), as[qs == .5], mean(as)))
    if (length(qs) == 1) {
        sigmas <- abs(mu - as)
    } else {
        sigmas <- mean((as[qs != .5] - mu) / qnorm(qs[qs != .5]))
    }
    scales <- 1
    ...
    for (ii in 1:min(1, (Nobs - 2))) {
        ...
        new.sigma <- (as[which.max(sqrerrs)] - new.mu) / qnorm(qs[which.max(sqrerrs)] - mu.q + .5)
    ...
    values <- c(values, rnorm(N - length(values), mus[1], sigmas[1]))
}
