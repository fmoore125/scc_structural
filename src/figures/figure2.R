...
for(i in 1:dim(struc)[2]) struc[,i]=fct_collapse(struc[,i],No=c("-1.0","0","-1",0),Yes=c("1.0","Calibrated","1",1))
colnames(struc)=paste0(colnames(struc),"_struc")

param=dat%>%
  select("TFP Growth":"Risk Aversion (EZ Utility)")%>%
  replace(is.na(.),0)
for(i in 1:dim(param)[2]) param[,i]=fct_collapse(param[,i],No=c("0",0),Yes=c("1.0",1))
...
if(length(which(distreg$row==todrop))>0) distreg=distreg[-which(distreg$row==todrop)]

#concatenate carbon cycle and climate model changes into a single "Earth System Change"
distreg$Earth_system_struc=factor(ifelse(distreg$"Carbon.Cycle_struc"=="Yes"|distreg$"Climate.Model_struc"=="Yes","Yes","No"))
